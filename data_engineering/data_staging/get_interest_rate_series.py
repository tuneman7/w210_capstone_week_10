...
series_dict = {'FPCPITOTLZGUSA':{'filename':'Inflation, consumer prices for the United States'},
...
historic_inflation  = pd.read_csv(output_dict["FPCPITOTLZGUSA"], index_col=False, parse_dates=['date'], date_parser=d)
historic_fed_rate   = pd.read_csv(output_dict["FEDFUNDS"], index_col=False, parse_dates=['date'], date_parser=d)
...
my_graph_data_object.label = "FED Series \"FPCPITOTLZGUSA\" \"Inflation, consumer prices for the United States\""
my_graph_data_object.x_axis_values = historic_inflation.sort_values(by=['date'])['date']
my_graph_data_object.y_axis_values = historic_inflation.sort_values(by=['date'])['value']
...
my_graph_data_object.x_axis_values = historic_fed_rate[(historic_fed_rate['date'] >= datetime.datetime(1960, 1, 1)) & (historic_fed_rate.index % 12==1)].sort_values(by=['date'])['date']
# print(my_graph_data_object.x_axis_values)
my_graph_data_object.y_axis_values = historic_fed_rate[(historic_fed_rate['date'] >= datetime.datetime(1960, 1, 1)) & (historic_fed_rate.index % 12==1)].sort_values(by=['date'])['value']

graph_label = "Historic Inflation Compared to Interest Rate"

list_of_graph_data_objects.append(my_graph_data_object)
my_graphing_object = GraphUtility()
my_graphing_object.plot_multi_line_graph_year_interval(list_of_graph_data_objects,"Historic Inflation Compared to Interest","Year","Value in Percentage")


x_axis_values = historic_fed_rate[(historic_fed_rate['date'] >= datetime.datetime(1960, 1, 1)) & (historic_fed_rate.index % 12==1)].sort_values(by=['date'])['date']
y_axis_values = historic_fed_rate[(historic_fed_rate['date'] >= datetime.datetime(1960, 1, 1)) & (historic_fed_rate.index % 12==1)].sort_values(by=['date'])['value']
x_label = "Year"
