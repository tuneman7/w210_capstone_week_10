...
core_data_directory = os.path.abspath(os.path.join(folder_location, os.pardir ,"datasets","realtor_msa"))
#RDC data (single file)
rdc_full_file_paths, rdc_file_names, rdc_loc_dict, rdc_data_frames = my_utility.view_file_types(file_filter="RDC_Inventory_Core_Metrics_Metro_History.csv",
                                             return_pandas_data_frames=True,
...
#now merge the stuff
rdc_data = rdc_data_frames["RDC_Inventory_Core_Metrics_Metro_History.csv"]

rdc_data["county_name"] = rdc_data["cbsa_title"]
...
sql = '''select distinct county_name from rdc_data'''

distinct_msa = psql.sqldf(sql)
...
sql ='select distinct county from '

#for each of the counties change it to county_name, yearmonth, value
distinct_msa_output = []
my_counter = 0
for key in distinct_msa["county_name"]:
    print("raw_key=",key)
    print("*"*60)
    county_name = key.split(',')[0]
#    county_name = key.replace("county","").replace(series_and_csv,"").replace(state,"").replace("unemployment_rate_in_","").replace("_"," ").replace("umployment rate in ","").replace("unemployment rate  ","").replace("unemploynt rate in ","").strip()
    ...
    for state_abr in key.split(',')[1].split('-'):
        print("state_abr=",state_abr.strip())
        #print("county_name=",county_name.strip())

    continue

    if "angeles" in county_name.lower():
        print("*"*60)
        print("county_name")
        print(county_name)
        print("raw_key")
...
folder_location = os.getcwd()
folder_location = os.path.abspath(os.path.join(folder_location, os.pardir ,"datasets","realtor_msa"))
...
file = os.path.join(folder_location,"unique_counties_msa.csv")
