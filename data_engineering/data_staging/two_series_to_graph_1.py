...
def output_file_from_list(filename,l):
...
def do_fed_extract_and_graph_multi_series(fred_series_object):
    ...
    list_non_fred_indexes = ['GOLDAMGBD228NLBM', 'djia', 'WALCL','NASDAQCOM','PCEPI1','DDDI06USA156NWDB']
    l_excel_file_names = []
    l_csv_file_names = []
    output_dict = {}
    series = Series()
    ...
    global series_count
    global l_series_lines
    for key in fred_series_object.series_dict.keys():
        if key in list_non_fred_indexes:
            continue
        key
        print(key)
        my_data_frame = series.get_series_df(key)
        my_series_object = series.get_a_series(key)["seriess"][0]
        # print(my_series_object)

        print("series_count=",series_count)

        if series_count == 0:
            series_count_line = "FED Series ID, Title, Freq, Observation Start, Observation End,Unit, Seasonal Adjustment"
            l_series_lines.append(series_count_line)
            series_count_line="{},{},{},{},{},{},{}".format(my_series_object["id"],
                                                            my_series_object["title"].replace(","," "),
                                                            my_series_object["frequency_short"],
                                                            my_series_object["observation_start"],
                                                            my_series_object["observation_end"],
                                                            my_series_object["units"],
                                                            my_series_object["seasonal_adjustment"]
                                                            )
            l_series_lines.append(series_count_line)
        else:
            series_count_line="{},{},{},{},{},{},{}".format(my_series_object["id"],
    ...
    for key in fred_series_object.series_dict.keys():
        ...
        if key in list_non_fred_indexes:
            ...
            if key=="WALCL" or key=="PCEPI1":
                d = lambda x: pd.datetime.strptime(x, '%Y-%m-%d')
            else:
                d = lambda x: pd.datetime.strptime(x, '%m/%d/%Y')

            csv = path.join(os.path.abspath(os.getcwd()),"fed_non_series_data","{}.csv".format(key))
            series_1 = pd.read_csv(csv, index_col=False, parse_dates=['date'], date_parser=d)
        ...
        if "linewidth" in  fred_series_object.series_dict[key].keys():
            my_graph_data_object.linewidth = fred_series_object.series_dict[key]["linewidth"]
        else:
...
fred_series_object.width = 15
fred_series_object.height = 7
...
fred_series_object.width = 15
fred_series_object.height = 7
fred_series_object.frequency_interval = 2
fred_series_object.frequency = "yearly"
...
fred_series_object.series_dict = {
    'CSUSHPINSA': {'filename': 'Case-Schiller U.S. National Price Index',
                 ...
                 'series_start': '1988-01-01',
                 'dollar_format': False},
    'MORTGAGE30US': {'filename': '30 Year Fixed Mortgage Interest Rates',
                  'y_label': '30 Year Fixed Mortgage Interest Rates',
                  'series_start': '1988-01-01',
                  'dollar_format': False
                  }
}

fred_series_object.graph_label = 'Case Schiller Home Index and 30 Year Fixed Mortgage Rates 1988 through Present'
fred_series_object.x_axis_label = 'Month and Year'
fred_series_object.width = 15
fred_series_object.height = 7
fred_series_object.frequency = "yearly"
...
fred_series_object.series_dict = {
    'GOLDAMGBD228NLBM': {'filename': 'Gold Price in U.S. Dollars',
                 'y_label': 'Gold Price',
                 'series_start': '1961-01-01',
                 'dollar_format': True},
    'CPIAUCSL': {'filename': 'Inflation Rate',
                  'y_label': 'Inflation Rate',
                  'series_start': '1961-01-01',
                  'dollar_format': False
                  }
...
fred_series_object.graph_label = 'Inflation Rate Compared to Gold Price'
fred_series_object.x_axis_label = 'Year'
fred_series_object.y_axis_label = 'Rate'
fred_series_object.width = 15
fred_series_object.height = 7
fred_series_object.frequency = "yearly"
fred_series_object.frequency_interval = 2
fred_series_object.linewidth = 2
fred_series_object.dual_axis = True
...
fred_series_object.graph_label = 'CPI Compared to FED funds rate Jan-2019 through Jun-2021'
fred_series_object.x_axis_label = 'Year'
fred_series_object.y_axis_label = 'Rate'
fred_series_object.width = 10
...
fred_series_object.frequency = "monthly"
fred_series_object.frequency_interval = 1
fred_series_object.linewidth = 3
...
fred_series_object.series_dict = {
    'WALCL': {'filename': 'Total Assets (Less Eliminations from Consolidation) (Quantitative Easing)',
                 'y_label': 'Total Fed Assets in Millions',
                 'series_start': '1996-04-01',
                 'dollar_format': True,
              'linewidth': 2},
    # 'GDPC1': {'filename': 'Real GDP Growth',
    #               'y_label': 'Real GDP Growth',
    #               'series_start': '1996-04-01',
    #               'dollar_format': True},
     'djia': {'filename': 'Dow Jones Industrial Average',
                  'y_label': 'Dow Jones Index',
                  'series_start': '1996-04-01',
                              'dollar_format': False
                  },
    'W270RE1A156NBEA': {'filename': 'Wage Earners Share of GDP',
                'y_label': 'Wage Earners Share of GDP',
                'series_start': '1996-04-01',
                'dollar_format': False,
                'linewidth': 5
                }
...
fred_series_object.graph_label = 'FED Quant Easing Stock Index Gains, Real Wage Gains'
fred_series_object.x_axis_label = 'Month and Year'
fred_series_object.width = 17
fred_series_object.height = 7
fred_series_object.frequency = "yearly"
fred_series_object.linewidth = 1
...
fred_series_object.series_dict = {
    'PCEPI1': {'filename': 'Personal Consumption Expenditures',
                 'y_label': 'Personal Consumption Expenditures',
                 'series_start': '1996-04-01',
                 'dollar_format': False}
}

fred_series_object.graph_label = 'Personal Consumption Expenditures U.S. Bureau of Economic Analysis'
fred_series_object.x_axis_label = 'Month and Year'
fred_series_object.width = 15
fred_series_object.height = 7
fred_series_object.frequency = "yearly"
...
fred_series_object.series_dict = {
    'DDDI06USA156NWDB': {'filename': 'FED Bank Assets to GDP',
                 'y_label': 'US FED Bank Assets as a Percentage of GDP',
                 'series_start': '1996-04-01',
                 'dollar_format': False},
    'WALCL': {'filename': 'Total Assets (Less Eliminations from Consolidation) (Quantitative Easing)',
              'y_label': 'Total Fed Assets in Millions',
...
fred_series_object.graph_label = 'U.S. Central Bank (FED) Assets, Quant Easing, and Money Supply'
fred_series_object.x_axis_label = 'Month and Year'
fred_series_object.width = 15
fred_series_object.height = 7
fred_series_object.frequency = "yearly"
...
output_file_from_list("series_graphed.csv",l_series_lines)
